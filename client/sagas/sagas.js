...
function* watchRedirect() {
    while (true) {
        const {request} = yield take(actions.REDIRECT);

        try {
            let params = {
                pathname: request.location ? request.location : location.pathname
            };

            if (request.params && Object.keys(request.params).length > 0) {
                params.search = queryString.stringify(request.params);
            }
